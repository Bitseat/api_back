...
router.post('/create-user', upload.array('avatar', 20), (req, res, next) => {
  ...
  for (var i = 0; i < req.files.length; i++) {
    ...
    try {
      fs.copyFileSync(path.join(__dirname, '../public/'+req.files[i].filename), 
                      path.join(__dirname, '../resumes/', req.files[i].filename))
  ...
  for (var i = 0; i < reqFiles.length; i++) {
    ...
    if(!fs.existsSync(path.join(__dirname, '../jsons/'+uploaded_file_name+'.json'))) {
        ...
        try {
          fs.copyFileSync(path.join(__dirname, '../'+uploaded_file_name.slice(0,uploaded_file_name.lastIndexOf('.')) + '.pdf'), 
                          path.join(__dirname, '../pdfs/'+uploaded_file_name.slice(0,uploaded_file_name.lastIndexOf('.')) + '.pdf'))
          console.log("Successfully copied and moved the file!")
        } catch(err) {
          throw err
        }
